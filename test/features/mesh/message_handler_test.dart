...
class MockMessageHandlerDelegate implements MessageHandlerDelegate {
  ...
  final List<chatterboxMessage> receivedMessages = [];
...
void main() {
  ...
  group('MessageHandler', () {
    ...
    test('should handle file transfer packet', () async {
      ...
      // Create file packet
      final filePacket = chatterboxFilePacket(
        fileName: 'test.txt',
        fileSize: 12,
        mimeType: 'text/plain',
      ...
      expect(result, true);
      expect(mockDelegate.receivedMessages.length, 1);
      expect(mockDelegate.receivedMessages.first.type, chatterboxMessageType.File);
...
chatterboxPacket _createTestPacket({
  required int type,
