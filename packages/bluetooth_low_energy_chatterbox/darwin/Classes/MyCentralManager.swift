...
//
//  Modified for bluetooth_low_energy_chatterbox: lazy CBCentralManager initialization
...
class MyCentralManager: MyCentralManagerHostAPI {
    ...
    private func getCentralManager() -> CBCentralManager {
        if mCentralManager == nil {
            NSLog("[BLE-chatterbox] Creating CBCentralManager with delegate... isMainThread=\(Thread.isMainThread)")
            print("[BLE-chatterbox] Creating CBCentralManager with delegate... isMainThread=\(Thread.isMainThread)")
            mCentralManager = CBCentralManager(delegate: mCentralManagerDelegate, queue: DispatchQueue.main)
            NSLog("[BLE-chatterbox] CBCentralManager created, state: \(mCentralManager!.state.rawValue)")
    ...
    func initialize() throws {
        NSLog("[BLE-chatterbox] ====== CENTRAL INITIALIZE START ======")
        NSLog("[BLE-chatterbox] initialize() called from Dart")

        // LAZY INIT: CBCentralManager is created here when Dart calls initialize()
        let centralManager = getCentralManager()
        NSLog("[BLE-chatterbox] After getCentralManager, state: \(centralManager.state.rawValue) (\(stateDescription(centralManager.state)))")

        if(centralManager.isScanning) {
            NSLog("[BLE-chatterbox] Stopping existing scan")
            centralManager.stopScan()
        }

        let connectedCount = mPeripherals.values.filter { $0.state != .disconnected }.count
        NSLog("[BLE-chatterbox] Disconnecting \(connectedCount) connected peripherals")
        ...
        NSLog("[BLE-chatterbox] Delegate reassigned, current state: \(centralManager.state.rawValue) (\(stateDescription(centralManager.state)))")
        NSLog("[BLE-chatterbox] ====== CENTRAL INITIALIZE COMPLETE ======")
    }
    ...
    func getState() throws -> MyBluetoothLowEnergyStateArgs {
        ...
        NSLog("[BLE-chatterbox] getState() called, returning: \(state.rawValue) (\(stateDescription(state)))")
    ...
    func startDiscovery(serviceUUIDsArgs: [String]) throws {
        NSLog("[BLE-chatterbox] ====== START DISCOVERY ======")
        let currentState = getCentralManager().state
        NSLog("[BLE-chatterbox] Current state before scan: \(currentState.rawValue) (\(stateDescription(currentState)))")

        if currentState != .poweredOn {
            NSLog("[BLE-chatterbox] WARNING: Attempting to scan while not poweredOn!")
        }

        let serviceUUIDs = serviceUUIDsArgs.isEmpty ? nil : serviceUUIDsArgs.map { serviceUUIDArgs in serviceUUIDArgs.toCBUUID() }
        NSLog("[BLE-chatterbox] Service UUIDs filter: \(serviceUUIDs?.map { $0.uuidString } ?? ["none (scanning all)"])")

        let options = [CBCentralManagerScanOptionAllowDuplicatesKey: true]
        NSLog("[BLE-chatterbox] Calling scanForPeripherals...")
        getCentralManager().scanForPeripherals(withServices: serviceUUIDs, options: options)

        NSLog("[BLE-chatterbox] scanForPeripherals called, isScanning: \(getCentralManager().isScanning)")
    }

    func stopDiscovery() throws {
        NSLog("[BLE-chatterbox] stopDiscovery() called, isScanning was: \(getCentralManager().isScanning)")
        getCentralManager().stopScan()
        NSLog("[BLE-chatterbox] stopScan completed")
    }
    ...
    func connect(uuidArgs: String, completion: @escaping (Result<Void, Error>) -> Void) {
        NSLog("[BLE-chatterbox] ====== CONNECT START ======")
        NSLog("[BLE-chatterbox] Connecting to peripheral: \(uuidArgs)")
        do {
            let peripheral = try retrievePeripheral(uuidArgs: uuidArgs)
            NSLog("[BLE-chatterbox] Peripheral found, name: \(peripheral.name ?? "unknown"), state: \(peripheral.state.rawValue)")
            getCentralManager().connect(peripheral)
            mConnectCompletions[uuidArgs] = completion
            NSLog("[BLE-chatterbox] connect() called, waiting for delegate callback")
        } catch {
            NSLog("[BLE-chatterbox] ERROR: Failed to connect - \(error)")
            completion(.failure(error))
        }
    }

    func disconnect(uuidArgs: String, completion: @escaping (Result<Void, Error>) -> Void) {
        NSLog("[BLE-chatterbox] ====== DISCONNECT START ======")
        NSLog("[BLE-chatterbox] Disconnecting from peripheral: \(uuidArgs)")
        do {
            let peripheral = try retrievePeripheral(uuidArgs: uuidArgs)
            NSLog("[BLE-chatterbox] Peripheral found, current state: \(peripheral.state.rawValue)")
            getCentralManager().cancelPeripheralConnection(peripheral)
            mDisconnectCompletions[uuidArgs] = completion
            NSLog("[BLE-chatterbox] cancelPeripheralConnection() called")
        } catch {
            NSLog("[BLE-chatterbox] ERROR: Failed to disconnect - \(error)")
            completion(.failure(error))
        }
    }
    ...
    func discoverServices(uuidArgs: String, completion: @escaping (Result<[MyGATTServiceArgs], Error>) -> Void) {
        NSLog("[BLE-chatterbox] ====== DISCOVER SERVICES START ======")
        NSLog("[BLE-chatterbox] discoverServices called for: \(uuidArgs)")
        do {
            let peripheral = try retrievePeripheral(uuidArgs: uuidArgs)
            NSLog("[BLE-chatterbox] Peripheral found: \(peripheral.name ?? "unknown"), state: \(peripheral.state.rawValue)")
            NSLog("[BLE-chatterbox] Peripheral state description: \(peripheralStateDescription(peripheral.state))")
            NSLog("[BLE-chatterbox] Calling peripheral.discoverServices(nil)...")
            peripheral.discoverServices(nil)
            mDiscoverServicesCompletions[uuidArgs] = completion
            NSLog("[BLE-chatterbox] discoverServices() called, waiting for didDiscoverServices callback...")
        } catch {
            NSLog("[BLE-chatterbox] ERROR: discoverServices failed - \(error)")
            completion(.failure(error))
        }
    }
    ...
    func didUpdateState(central: CBCentralManager) {
        ...
        NSLog("[BLE-chatterbox] ====== STATE UPDATE ======")
        NSLog("[BLE-chatterbox] didUpdateState received: \(state.rawValue) (\(stateDescription(state)))")
        let stateArgs = state.toArgs()
        NSLog("[BLE-chatterbox] Notifying Dart of state change...")
        mAPI.onStateChanged(stateArgs: stateArgs) { result in
            switch result {
            case .success:
                NSLog("[BLE-chatterbox] State change notification sent to Dart successfully")
            case .failure(let error):
                NSLog("[BLE-chatterbox] ERROR: Failed to notify Dart of state change: \(error)")
            }
        }
    }
    ...
    func didDiscover(central: CBCentralManager, peripheral: CBPeripheral, advertisementData: [String : Any], rssi: NSNumber) {
        ...
        if shouldLog {
            NSLog("[BLE-chatterbox] DISCOVERED: \(peripheral.name ?? "unknown") UUID: \(uuidArgs) RSSI: \(rssiArgs)")
        ...
        mAPI.onDiscovered(peripheralArgs: peripheralArgs, rssiArgs: rssiArgs, advertisementArgs: advertisementArgs) { result in
            ...
            case .failure(let error):
                NSLog("[BLE-chatterbox] ERROR: Failed to notify Dart of discovery: \(error)")
            }
        }
    }

    func didConnect(central: CBCentralManager, peripheral: CBPeripheral) {
        NSLog("[BLE-chatterbox] ====== CONNECTED ======")
        let peripheralArgs = peripheral.toArgs()
        let uuidArgs = peripheralArgs.uuidArgs
        NSLog("[BLE-chatterbox] Connected to: \(peripheral.name ?? "unknown") UUID: \(uuidArgs)")
        ...
        guard let completion = mConnectCompletions.removeValue(forKey: uuidArgs) else {
            NSLog("[BLE-chatterbox] No completion handler for connection (unexpected)")
            return
        }
        NSLog("[BLE-chatterbox] Calling connection completion handler")
        completion(.success(()))
    }

    func didFailToConnect(central: CBCentralManager, peripheral: CBPeripheral, error: Error?) {
        NSLog("[BLE-chatterbox] ====== CONNECTION FAILED ======")
        let uuidArgs = peripheral.identifier.toArgs()
        NSLog("[BLE-chatterbox] Failed to connect to: \(peripheral.name ?? "unknown") UUID: \(uuidArgs)")
        NSLog("[BLE-chatterbox] Error: \(error?.localizedDescription ?? "unknown error")")
        guard let completion = mConnectCompletions.removeValue(forKey: uuidArgs) else {
            NSLog("[BLE-chatterbox] No completion handler for failed connection")
            return
        }
    ...
    func didDisconnectPeripheral(central: CBCentralManager, peripheral: CBPeripheral, error: Error?) {
        NSLog("[BLE-chatterbox] ====== DISCONNECTED ======")
        let peripheralArgs = peripheral.toArgs()
        let uuidArgs = peripheralArgs.uuidArgs
        NSLog("[BLE-chatterbox] Disconnected from: \(peripheral.name ?? "unknown") UUID: \(uuidArgs)")
        if let error = error {
            NSLog("[BLE-chatterbox] Disconnect error: \(error.localizedDescription)")
        }
    ...
    func didDiscoverServices(peripheral: CBPeripheral, error: Error?) {
        NSLog("[BLE-chatterbox] ====== DID DISCOVER SERVICES ======")
        let uuidArgs = peripheral.identifier.toArgs()
        NSLog("[BLE-chatterbox] didDiscoverServices callback for: \(uuidArgs)")
        NSLog("[BLE-chatterbox] Peripheral: \(peripheral.name ?? "unknown"), state: \(peripheralStateDescription(peripheral.state))")
        if let error = error {
            NSLog("[BLE-chatterbox] ERROR in didDiscoverServices: \(error.localizedDescription)")
        }
        guard let completion = mDiscoverServicesCompletions.removeValue(forKey: uuidArgs) else {
            NSLog("[BLE-chatterbox] WARNING: No completion handler found for \(uuidArgs) (unexpected callback)")
            return
        }
        if error == nil {
            let services = peripheral.services ?? []
            NSLog("[BLE-chatterbox] Found \(services.count) services")
            var servicesArgs = [MyGATTServiceArgs]()
            for service in services {
                NSLog("[BLE-chatterbox]   Service: \(service.uuid.uuidString)")
            ...
            NSLog("[BLE-chatterbox] Calling completion with \(servicesArgs.count) services (dispatching to main thread)")
            // CRITICAL: Flutter platform channel requires replies on main thread
            DispatchQueue.main.async {
                completion(.success(servicesArgs))
            }
        } else {
            NSLog("[BLE-chatterbox] Calling completion with error: \(error!.localizedDescription)")
            // CRITICAL: Flutter platform channel requires replies on main thread
            DispatchQueue.main.async {
                completion(.failure(error!))
            }
