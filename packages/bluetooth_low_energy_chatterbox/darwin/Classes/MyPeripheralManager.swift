...
//
//  Modified for bluetooth_low_energy_chatterbox: lazy CBPeripheralManager initialization
...
class MyPeripheralManager: MyPeripheralManagerHostAPI {
    ...
    private func getPeripheralManager() -> CBPeripheralManager {
        if mPeripheralManager == nil {
            NSLog("[BLE-chatterbox-Peripheral] Creating CBPeripheralManager with delegate...")
            mPeripheralManager = CBPeripheralManager(delegate: mPeripheralManagerDelegate, queue: nil)
            NSLog("[BLE-chatterbox-Peripheral] CBPeripheralManager created, state: \(mPeripheralManager!.state.rawValue) (\(stateDescription(mPeripheralManager!.state)))")
        }
    ...
    func initialize() throws {
        NSLog("[BLE-chatterbox-Peripheral] ====== PERIPHERAL INITIALIZE START ======")

        // LAZY INIT: CBPeripheralManager is created here when Dart calls initialize()
        let peripheralManager = getPeripheralManager()
        NSLog("[BLE-chatterbox-Peripheral] After getPeripheralManager, state: \(peripheralManager.state.rawValue) (\(stateDescription(peripheralManager.state)))")

        if(peripheralManager.isAdvertising) {
            NSLog("[BLE-chatterbox-Peripheral] Stopping existing advertising")
        ...
        NSLog("[BLE-chatterbox-Peripheral] Delegate reassigned")
        NSLog("[BLE-chatterbox-Peripheral] ====== PERIPHERAL INITIALIZE COMPLETE ======")
    }

    func getState() throws -> MyBluetoothLowEnergyStateArgs {
        let state = getPeripheralManager().state
        NSLog("[BLE-chatterbox-Peripheral] getState() called, returning: \(state.rawValue) (\(stateDescription(state)))")
    ...
    func addService(serviceArgs: MyMutableGATTServiceArgs, completion: @escaping (Result<Void, Error>) -> Void) {
        NSLog("[BLE-chatterbox-Peripheral] ====== ADD SERVICE ======")
        NSLog("[BLE-chatterbox-Peripheral] Adding service UUID: \(serviceArgs.uuidArgs)")
        do {
            let service = try addServiceArgs(serviceArgs)
            NSLog("[BLE-chatterbox-Peripheral] Service created, adding to peripheral manager...")
            getPeripheralManager().add(service)
            mAddServiceCompletion = completion
            NSLog("[BLE-chatterbox-Peripheral] add(service) called, waiting for callback")
        } catch {
            NSLog("[BLE-chatterbox-Peripheral] ERROR: Failed to add service - \(error)")
            completion(.failure(error))
        }
    }
    ...
    func startAdvertising(advertisementArgs: MyAdvertisementArgs, completion: @escaping (Result<Void, Error>) -> Void) {
        NSLog("[BLE-chatterbox-Peripheral] ====== START ADVERTISING ======")
        let currentState = getPeripheralManager().state
        NSLog("[BLE-chatterbox-Peripheral] Current state: \(currentState.rawValue) (\(stateDescription(currentState)))")
        if currentState != .poweredOn {
            NSLog("[BLE-chatterbox-Peripheral] WARNING: Attempting to advertise while not poweredOn!")
        }
        let advertisement = advertisementArgs.toAdvertisement()
        NSLog("[BLE-chatterbox-Peripheral] Advertisement data: \(advertisement)")
        getPeripheralManager().startAdvertising(advertisement)
        mStartAdvertisingCompletion = completion
        NSLog("[BLE-chatterbox-Peripheral] startAdvertising() called, waiting for callback")
    }

    func stopAdvertising() throws {
        NSLog("[BLE-chatterbox-Peripheral] stopAdvertising() called")
    ...
    func didUpdateState(peripheral: CBPeripheralManager) {
        ...
        NSLog("[BLE-chatterbox-Peripheral] ====== STATE UPDATE ======")
        NSLog("[BLE-chatterbox-Peripheral] didUpdateState: \(state.rawValue) (\(stateDescription(state)))")
        let stateArgs = state.toArgs()
        NSLog("[BLE-chatterbox-Peripheral] Notifying Dart of peripheral state change...")
        mAPI.onStateChanged(stateArgs: stateArgs) { result in
            switch result {
            case .success:
                NSLog("[BLE-chatterbox-Peripheral] State change notification sent to Dart")
            case .failure(let error):
                NSLog("[BLE-chatterbox-Peripheral] ERROR: Failed to notify Dart - \(error)")
            }
        }
    }

    func didAdd(peripheral: CBPeripheralManager, service: CBService, error: Error?) {
        NSLog("[BLE-chatterbox-Peripheral] ====== SERVICE ADDED CALLBACK ======")
        NSLog("[BLE-chatterbox-Peripheral] Service: \(service.uuid.uuidString)")
        if let error = error {
            NSLog("[BLE-chatterbox-Peripheral] ERROR: \(error.localizedDescription)")
        }
        guard let completion = mAddServiceCompletion else {
            NSLog("[BLE-chatterbox-Peripheral] No completion handler for addService")
            return
        }
        mAddServiceCompletion = nil
        if error == nil {
            NSLog("[BLE-chatterbox-Peripheral] Service added successfully")
            completion(.success(()))
        } else {
            completion(.failure(error!))
        }
    ...
    func didStartAdvertising(peripheral: CBPeripheralManager, error: Error?) {
        NSLog("[BLE-chatterbox-Peripheral] ====== ADVERTISING STARTED CALLBACK ======")
        if let error = error {
            NSLog("[BLE-chatterbox-Peripheral] ERROR: \(error.localizedDescription)")
        } else {
            NSLog("[BLE-chatterbox-Peripheral] Advertising started successfully")
        }
        guard let completion = mStartAdvertisingCompletion else {
            NSLog("[BLE-chatterbox-Peripheral] No completion handler for startAdvertising")
            return
        }
        mStartAdvertisingCompletion = nil
        if error == nil {
