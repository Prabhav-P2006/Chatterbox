...
class MyPeripheralDelegate: NSObject, CBPeripheralDelegate {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        NSLog("[BLE-chatterbox] DELEGATE: didDiscoverServices called for \(peripheral.identifier.uuidString)")
        if let error = error {
            NSLog("[BLE-chatterbox] DELEGATE: didDiscoverServices error: \(error.localizedDescription)")
        } else {
            NSLog("[BLE-chatterbox] DELEGATE: didDiscoverServices success, services count: \(peripheral.services?.count ?? 0)")
        }
