...
import 'package:chatterbox/data/models/chatterbox_file_packet.dart';
import 'package:chatterbox/features/mesh/bluetooth_mesh_service.dart';
import 'package:chatterbox/features/mesh/peer_manager.dart' as pm;
import 'package:chatterbox/features/mesh/message_handler.dart';
import 'package:chatterbox/features/mesh/store_forward_manager.dart';
import 'package:chatterbox/features/storage/message_storage_service.dart';
import 'package:chatterbox/protocol/message_type.dart';
...
class ChatState {
  ...
  final List<chatterboxMessage> messages;
  ...
  ChatState copyWith({
    List<chatterboxMessage>? messages,
...
class ChatNotifier extends StateNotifier<ChatState>
  ...
  Future<void> _persistMessage(chatterboxMessage message) async {
    try {
  ...
  void didReceiveDeliveryAck(String messageID, String recipientPeerID) {
    ...
    final messages = List<chatterboxMessage>.from(state.messages);
  ...
  Future<void> _sendPrivateMessage(String content, String peerId) async {
    ...
    final message = chatterboxMessage(
  ...
  void addMessage(chatterboxMessage message) {
    _addMessage(message);
  ...
  void _addMessage(chatterboxMessage message) {
    ...
    final updatedMessages = List<chatterboxMessage>.from(state.messages);
  ...
  void addSystemMessage(String content) {
    final systemMessage = chatterboxMessage(
      id: _generateMessageId(),
      sender: 'system',
      content: content,
      timestamp: DateTime.now(),
  ...
  void _updateUnreadCounts(chatterboxMessage message) {
    if (message.isPrivate && message.senderPeerID != null) {
...
class _MessageHandlerDelegateImpl implements MessageHandlerDelegate {
  ...
  @override
  Future<bool> verifySignature(chatterboxPacket packet, String peerID) async {
    // Delegate to encryption service if needed
    return false;
  }
  ...
  @override
  Future<chatterboxPacket?> handleFragment(chatterboxPacket packet) async {
    // Handle fragmentation if needed
    return null;
  }

  @override
  Future<String> saveIncomingFile(chatterboxFilePacket file) async {
  ...
  @override
  void onMessageReceived(chatterboxMessage message) {
