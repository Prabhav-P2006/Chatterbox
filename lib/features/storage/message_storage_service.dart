...
class MessageStorageService {
  ...
  List<chatterboxMessage> getPublicMessages() {
    if (!_initialized || _publicBox == null) return [];

    try {
      return _publicBox!.values
          .map((json) => chatterboxMessage.fromJson(jsonDecode(json)))
          .toList();
    } catch (e) {
  ...
  List<chatterboxMessage> getPrivateMessages(String peerID) {
    return _getPrivateMessagesForPeer(_normalizeKey(peerID));
  }

  /// Get all private messages grouped by peer ID.
  Map<String, List<chatterboxMessage>> getAllPrivateMessages() {
    if (!_initialized || _privateBox == null) return {};

    try {
      final result = <String, List<chatterboxMessage>>{};
  ...
  List<chatterboxMessage> _getPrivateMessagesForPeer(String key) {
    if (!_initialized || _privateBox == null) return [];
    ...
    try {
      ...
      return list.map((m) => chatterboxMessage.fromJson(m)).toList();
    } catch (e) {
      debugPrint(
  ...
  List<chatterboxMessage> getChannelMessages(String channel) {
    return _getChannelMessagesForChannel(_normalizeKey(channel));
  }

  /// Get all channel messages grouped by channel name.
  Map<String, List<chatterboxMessage>> getAllChannelMessages() {
  ...
  List<chatterboxMessage> _getChannelMessagesForChannel(String key) {
    if (!_initialized || _channelBox == null) return [];
