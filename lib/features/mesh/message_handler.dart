...
class MessageHandler {
  ...
  bool _isPacketAddressedToMe(chatterboxPacket packet) {
  ...
  Future<bool> _handleBroadcastMessage(RoutedPacket routed) async {
    ...
    try {
      ...
      if (messageType == MessageType.fileTransfer) {
        final file = chatterboxFilePacket.decode(payload);
        if (file != null) {
  ...
  Future<bool> _handleNoisePayload(
    ...
    String peerID,
    chatterboxPacket originalPacket,
  ) async {
    try {
      switch (noisePayload.type) {
        ...
        case 0x02: // FILE_TRANSFER
          final file = chatterboxFilePacket.decode(noisePayload.data);
          if (file != null) {
            return await _handleFilePacket(
              file,
              peerID,
  ...
  Uint8List? _getPacketDataForSigning(chatterboxPacket packet) {
    try {
      // Create a new packet without signature for signing
      final signPacket = chatterboxPacket(
  ...
  chatterboxMessageType _getMessageTypeForMime(String? mimeType) {
    if (mimeType == null) return chatterboxMessageType.Message;

    if (mimeType.startsWith('audio/')) {
      return chatterboxMessageType.Audio;
    } else if (mimeType.startsWith('image/')) {
      return chatterboxMessageType.Image;
    } else {
      return chatterboxMessageType.File;
    }
  }
...
abstract class MessageHandlerDelegate {
  ...
  Future<bool> verifySignature(chatterboxPacket packet, String peerID);
  ...
  /// Fragment handling
  Future<chatterboxPacket?> handleFragment(chatterboxPacket packet);

  /// File operations
  Future<String> saveIncomingFile(chatterboxFilePacket file);
