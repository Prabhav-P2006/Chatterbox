...
import 'package:chatterbox/features/crypto/encryption_service.dart';
import 'package:chatterbox/features/mesh/ble_manager.dart';
import 'package:chatterbox/features/mesh/gatt_server_manager.dart';
import 'package:chatterbox/features/mesh/gatt_client_manager.dart';
import 'package:chatterbox/features/mesh/peer_manager.dart';
import 'package:chatterbox/features/mesh/store_forward_manager.dart';
import 'package:chatterbox/features/mesh/power_manager.dart';
import 'package:chatterbox/data/models/chatterbox_message.dart';
import 'package:chatterbox/data/models/routed_packet.dart';
import 'package:chatterbox/data/models/identity_announcement.dart';
import 'package:chatterbox/protocol/packet_codec.dart';
import 'package:chatterbox/core/constants.dart';
...
abstract class BluetoothMeshDelegate {
  ...
  void didReceiveMessage(chatterboxMessage message);
...
class BluetoothMeshService
  ...
  /// @param packet - The chatterboxPacket to broadcast
  /// @return Number of peers the packet was sent to
  Future<int> broadcastPacket(chatterboxPacket packet) async {
  ...
  /// @param packet - The chatterboxPacket to send
  /// @return true if the packet was sent successfully
  Future<bool> sendPacketToPeer(String peerID, chatterboxPacket packet) async {
    if (!_isActive) {
      return false;
    }
  ...
  String _createPacketKey(chatterboxPacket packet) {
  ...
  Future<chatterboxPacket> _signPacket(chatterboxPacket packet) async {
    try {
  ...
  void onDataReceived(Uint8List data, String deviceAddress) {
    ...
    // Decode the received data as a chatterboxPacket
    debugPrint('$_tag Attempting to decode packet...');
    final packet = chatterboxPacket.decode(data);
    if (packet == null) {
  ...
  void _processIncomingPacket(chatterboxPacket packet, String peerID) {
    debugPrint('$_tag ========================================');
  ...
  void _processIdentityAnnouncement(chatterboxPacket packet, String peerID) {
    try {
  ...
  void _processChatMessage(chatterboxPacket packet, String peerID) {
    debugPrint('$_tag ========================================');
    ...
    try {
      ...
      debugPrint('$_tag Calling chatterboxMessage.fromBinaryPayload...');
      final msg = chatterboxMessage.fromBinaryPayload(packet.payload!);
