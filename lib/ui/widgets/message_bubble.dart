...
import 'package:chatterbox/ui/theme/chatterbox_colors.dart';
import 'package:chatterbox/ui/theme/chatterbox_theme.dart';
import 'package:chatterbox/ui/theme/chatterbox_typography.dart';
import 'package:chatterbox/ui/utils/chat_ui_utils.dart';
...
class MessageBubble extends StatelessWidget {
  ...
  final chatterboxMessage message;
  ...
  Widget build(BuildContext context) {
    ...
    final isDark = chatterboxTheme.isDarkTheme(context);
    final isOwnMessage = message.sender == currentUserNickname;

    // System messages use different formatting
    if (message.type == chatterboxMessageType.Message && _isSystemMessage) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: RichText(
          text: TextSpan(
            style: chatterboxTypography.messageContentStyle(colorScheme),
  ...
  List<InlineSpan> _buildIrcMessageSpans(
    ...
    if (isOwnMessage) {
      peerColor = chatterboxColors.selfMessageColor; // Orange for self
    } else {
    ...
    spans.add(TextSpan(
      text: '<@',
      style: chatterboxTypography.senderNicknameStyle(peerColor),
    ));
    ...
    if (onNicknameTap != null && !isOwnMessage) {
      spans.add(WidgetSpan(
        ...
        child: GestureDetector(
          ...
          child: Text(
            truncatedBase,
            style: chatterboxTypography.senderNicknameStyle(peerColor).copyWith(
    ...
    if (suffix.isNotEmpty) {
      spans.add(TextSpan(
        text: suffix,
        style: chatterboxTypography.hashSuffixStyle(peerColor),
      ));
    }
    ...
    spans.add(TextSpan(
      text: timeStr,
      style: chatterboxTypography.timestampStyle(colorScheme),
    ));
    ...
    for (final match in allMatches) {
      ...
      if (matchedText.startsWith('@')) {
        ...
        final mentionColor = isSelfMention
            ? chatterboxColors.selfMessageColor
        ...
        spans.add(TextSpan(
          text: matchedText,
          style: TextStyle(
            color: chatterboxColors.linkColor,
  ...
  TextSpan _buildDeliveryStatusSpan(ColorScheme colorScheme) {
    ...
    status.map(
      ...
      read: (_) {
        statusStr = ' \u2713\u2713'; // ✓✓
        statusColor = chatterboxColors.linkColor;
      },
      failed: (_) {
        statusStr = ' \u2717'; // ✗
  ...
  TextSpan _buildRssiSpan() {
    ...
    final rssiColor = chatterboxColors.colorForRssi(rssi!);
    return TextSpan(
      text: ' \u26E8', // ⛨ shield symbol
  ...
  Widget _buildSystemMessage(BuildContext context, ColorScheme colorScheme) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: chatterboxTypography.systemMessageStyle(colorScheme),
