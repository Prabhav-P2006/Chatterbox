...
part 'chatterbox_message.freezed.dart';
part 'chatterbox_message.g.dart';

enum chatterboxMessageType { Message, Audio, Image, File }
...
@freezed
class chatterboxMessage with _$chatterboxMessage {
  const factory chatterboxMessage({
    @Default('') String id,
    required String sender,
    @Default('') String content,
    @Default(chatterboxMessageType.Message) chatterboxMessageType type,
  ...
  factory chatterboxMessage.fromJson(Map<String, dynamic> json) =>
      _$chatterboxMessageFromJson(json);

  // Binary serialization compatible with Android implementation
  static Uint8List? toBinaryPayload(chatterboxMessage msg) {
    try {
  ...
  static chatterboxMessage? fromBinaryPayload(Uint8List data) {
    debugPrint('[chatterboxMessage] ========================================');
    debugPrint('[chatterboxMessage] fromBinaryPayload CALLED');
    debugPrint('[chatterboxMessage] Input size: ${data.length} bytes');
    debugPrint('[chatterboxMessage] First 30 bytes: ${data.take(30).toList()}');
    debugPrint('[chatterboxMessage] ========================================');

    try {
      if (data.isEmpty) {
        debugPrint('[chatterboxMessage] FAIL: data is empty');
        return null;
      }

      // Try structured binary format first (Flutter-to-Flutter)
      if (data.length >= 13) {
        debugPrint('[chatterboxMessage] Trying structured format (len >= 13)...');
        final structured = _parseStructuredPayload(data);
        if (structured != null) {
          debugPrint('[chatterboxMessage] Structured format SUCCESS');
          debugPrint(
              '[chatterboxMessage] Parsed content: "${structured.content}"');
          return structured;
        }
        debugPrint('[chatterboxMessage] Structured format FAILED');
      }

      // Fallback: Android sends plain UTF-8 text as payload
      // Try to decode as plain UTF-8 string
      debugPrint('[chatterboxMessage] Trying plain UTF-8 fallback...');
      try {
        final content = utf8.decode(data, allowMalformed: false);
        debugPrint('[chatterboxMessage] UTF-8 decode SUCCESS');
        debugPrint('[chatterboxMessage] Content: "$content"');
        ...
        debugPrint('[chatterboxMessage] Generated unique ID: $uniqueId');

        return chatterboxMessage(
          id: uniqueId,
          sender: '', // Will be filled by caller from packet senderID
          content: content,
          type: chatterboxMessageType.Message,
          timestamp: now,
        );
      } catch (e) {
        debugPrint('[chatterboxMessage] UTF-8 decode FAILED: $e');
        debugPrint(
            '[chatterboxMessage] Raw bytes as hex: ${data.map((b) => b.toRadixString(16).padLeft(2, '0')).join(' ')}');
        // Not valid UTF-8, return null
        return null;
      }
    } catch (e, stack) {
      debugPrint('[chatterboxMessage] EXCEPTION: $e');
      debugPrint('[chatterboxMessage] Stack: $stack');
      return null;
    }
  }

  /// Parse the structured binary format (Flutter-to-Flutter messages)
  static chatterboxMessage? _parseStructuredPayload(Uint8List data) {
    try {
      ...
      debugPrint(
          '[chatterboxMessage._parseStructured] Flags: 0x${flags.toRadixString(16)}');
      ...
      debugPrint(
          '[chatterboxMessage._parseStructured] isRelay=$isRelay, isPrivate=$isPrivate, hasChannel=$hasChannel, isEncrypted=$isEncrypted');

      final ts = buf.getInt64(offset, Endian.big);
      offset += 8;
      final timestamp = DateTime.fromMillisecondsSinceEpoch(ts);
      debugPrint('[chatterboxMessage._parseStructured] Timestamp: $timestamp');

      final idLength = buf.getUint8(offset);
      offset += 1;
      debugPrint('[chatterboxMessage._parseStructured] ID length: $idLength');
      if (data.length < offset + idLength) {
        debugPrint(
            '[chatterboxMessage._parseStructured] FAIL: not enough data for ID');
        return null;
      }
      final id = utf8.decode(data.sublist(offset, offset + idLength));
      offset += idLength;
      debugPrint('[chatterboxMessage._parseStructured] ID: "$id"');

      final senderLength = buf.getUint8(offset);
      offset += 1;
      debugPrint(
          '[chatterboxMessage._parseStructured] Sender length: $senderLength');
      if (data.length < offset + senderLength) {
        debugPrint(
            '[chatterboxMessage._parseStructured] FAIL: not enough data for sender');
        return null;
      }
      final sender = utf8.decode(data.sublist(offset, offset + senderLength));
      offset += senderLength;
      debugPrint('[chatterboxMessage._parseStructured] Sender: "$sender"');

      final contentLength = buf.getUint16(offset, Endian.big);
      offset += 2;
      debugPrint(
          '[chatterboxMessage._parseStructured] Content length: $contentLength');
      if (data.length < offset + contentLength) {
        debugPrint(
            '[chatterboxMessage._parseStructured] FAIL: not enough data for content');
        return null;
      }
      ...
      if (isEncrypted) {
        ...
        debugPrint(
            '[chatterboxMessage._parseStructured] Encrypted content: ${encryptedContent.length} bytes');
      } else {
        content = utf8.decode(data.sublist(offset, offset + contentLength));
        offset += contentLength;
        debugPrint('[chatterboxMessage._parseStructured] Content: "$content"');
      }
      ...
      if (hasChannel && offset < data.length) {
        ...
        if (data.length >= offset + len) {
          ...
          offset += len;
          debugPrint('[chatterboxMessage._parseStructured] Channel: "$channel"');
        }
      }

      debugPrint(
          '[chatterboxMessage._parseStructured] SUCCESS - returning message');
      return chatterboxMessage(
        id: id,
        sender: sender,
        content: content,
      ...
      debugPrint('[chatterboxMessage._parseStructured] EXCEPTION: $e');
      debugPrint('[chatterboxMessage._parseStructured] Stack: $stack');
      return null;
    }
  }
}
